...
// ─── 채널별 콘텐츠 생성 (메인 함수) ──────────────────────────
async function generateContent(channel: ChannelKey, topic: string, draft: string, systemPrompt: string): Promise<string> {
...
export async function POST(req: NextRequest) {
  try {
    const { topic, draft = "", channels: requestedChannels } = await req.json();
    ...
    const results = await Promise.all(
      targetChannels.map(async (channel) => {
        ...
        const content = await generateContent(channel, topic.trim(), draft, systemPrompt);
